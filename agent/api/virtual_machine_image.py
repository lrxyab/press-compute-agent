...
if TYPE_CHECKING:
	from agent.agent.doctype.virtual_machine_image.virtual_machine_image import VirtualMachineImage


@frappe.whitelist()
def new(instance_id: str):
	virtual_machine = frappe.db.get_value("Virtual Machine", {"uuid": instance_id})
	virtual_machine_image_doc: VirtualMachineImage = frappe.new_doc("Virtual Machine Image")
	...
	virtual_machine_image_doc.take_image()
